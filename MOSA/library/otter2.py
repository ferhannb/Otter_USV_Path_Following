...
if __name__ == "__main__": 
    ...
    heading_rate = []
    avarage_radius=[]
    for j in np.linspace(100,-100,41):

        for k in np.linspace(-100,100,41):
            current_eta = np.array([0, 00, 0, 0, 0, math.radians(0)], float)
            nu = np.array([0, 0, 0, 0, 0, 0], float)
            for _ in range(500):
                ...
                print(radius)
                d_list.append(radius)

            
            iskele.append(u_control[1])
            sancak.append(u_control[0])
            heading_rate.append(nu[5])
            avr_r = d_list[-1]
            avarage_radius.append(avr_r)
            print(k)
            print(j)
...
# dictionary of lists  
import pandas as pd
dict = {'iskele': iskele,'sancak':sancak, 'avarage_radius': avarage_radius, 'heading_rate': heading_rate}  
       
df = pd.DataFrame(dict) 
    
# saving the dataframe 
df.to_csv('otterRadiuesValues.csv')
