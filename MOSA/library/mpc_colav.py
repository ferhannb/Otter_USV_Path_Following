...
class MPCUSV_colav(TargetVehicle):
    ...
    def __init__(self, colreg,x_obs,y_obs,N=40, dt = 0.15):
        TargetVehicle.__init__(self)
        ...
        for j in range(self.N+1):
            g = ca.vertcat(g,-ca.sqrt((self.x_obs-X[3,j])**2+(self.y_obs-X[4,j])**2)+(diam_obs/2+diam_usv/2+1))

        if colreg=='Head-on':

            for z in range(self.N+1):
                g = ca.vertcat(g,30-X[3,z])   # HEAD-ON COLREG için Gerekli olan kısıt
        ...
        lbx[0: self.n_states*(N+1): self.n_states] = -6         # u lower bound
        lbx[1: self.n_states*(N+1): self.n_states] = -6         # v lower bound
        lbx[2: self.n_states*(N+1): self.n_states] = -ca.inf     # r lower bound
        ...
        ubx[0: self.n_states*(N+1): self.n_states] = 6         # u lower bound
        ubx[1: self.n_states*(N+1): self.n_states] = 6         # v lower bound
        ubx[2: self.n_states*(N+1): self.n_states] = ca.inf     # r lower bound
        ...
        lbg = ca.DM.zeros(self.n_states*(self.N+1)+self.N+1,1)
        ubg = ca.DM.zeros(self.n_states*(self.N+1)+self.N+1,1)

        

        lbg[self.n_states*(self.N+1):] = -ca.inf
        ubg[self.n_states*(self.N+1):] = 0.0

        if colreg=='Head-on':
            lbg = ca.DM.zeros(self.n_states*(self.N+1)+self.N+1+self.N+1,1)
            ubg = ca.DM.zeros(self.n_states*(self.N+1)+self.N+1+self.N+1,1)

            lbg[self.n_states*(self.N+1)+self.N+1:] = -20
            ubg[self.n_states*(self.N+1)+self.N+1:] = 0
    ...
    def execute_MPC(self,init_states=[0,0,0,0,0,math.pi/2],target_states=[0,0,0,10,10,0]):
        ...
        output = dict(state_history=state_history,usv_states=xx,t=t,init_states=init_states,target_states=target_states,home_pose=home_pose,control_signal=u[:,0])
        return output
